...
/// wwwroot/files 폴더를 기준으로 Blob 컨테이너와 동기화합니다.
/// </summary>
[ApiController]
[Route("[controller]")]
    ...
    IConfiguration configuration) : ControllerBase
{
    ...
    /// <summary>
    /// 지정된 로컬 경로의 모든 파일을 Blob Storage로 업로드합니다.
    /// 하위 디렉터리까지 모두 포함합니다.
    /// 
    /// 동일한 Blob이 존재하는 경우:
    /// - Blob 파일 크기가 0바이트이고
    ...
    /// - 그 외에는 업로드를 생략합니다.
    /// </summary>
    /// <param name="localPath">로컬 파일 루트 경로</param>
    private async Task UploadFilesToBlobAsync(string localPath)
    {
        ...
        foreach (var filePath in Directory.GetFiles(localPath, "*", SearchOption.AllDirectories))
        {
            ...
            if (await blobClient.ExistsAsync())
            {
                ...
                // Blob이 0바이트이고 로컬 파일이 정상 크기라면 재업로드
    ...
    /// <summary>
    /// Blob Storage의 모든 파일을 로컬 경로로 다운로드합니다.
    /// 
    /// - 하위 디렉터리 구조를 유지합니다.
    /// - 로컬에 동일 파일이 존재하면 다운로드를 생략합니다.
    /// </summary>
    /// <param name="localPath">로컬 저장 경로</param>
